...
local function isIntegral(number)
...
if not jit then
    ...
    local chunk = "a = 1"
    loader(chunk)()  -- Execute the chunk

    -- Now, inspect the behavior of numbers
    local testNumber = 1.5
    if isIntegral(testNumber) then
        NUMBER_INTEGRAL = true
    end
else
    -- For LuaJIT or other environments where jit is defined
    -- Adjust SIZEOF_NUMBER and NUMBER_INTEGRAL as needed for your environment
    SIZEOF_NUMBER = 8
